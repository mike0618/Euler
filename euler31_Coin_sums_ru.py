...
start = time()
coins = (2, 5, 10, 50, 100, 200, 500, 1000, 2000)


def e31ru(amount, maxc):
    ways = 0
    for coin in coins:
        if coin < maxc:  # take coins less then present to correct completion of remainders
            ...
            if coin > 2:  # here are more ways for coins greater then 2
                ...
                for n in range(ceil(amount / coin) - 1):
                    ...
                    ways += e31ru(remain, coin)  # run recursion for each remainder to figure out all the ways

    return ways


print("Number of ways =", e31ru(2000, 2001) + 1)  # 73682 (+ 1 is for 200 * 1p way, we didn't count it in e31 function)
end = time() - start
print("Runtime =", end)
